...
and make a comment post and delete it
    ...
    Background: Preconditions
        ...
        * def slugResponse = callonce read('classpath:helpers/slug.feature')
    ...
    Scenario: Favorite articles
        ...
        * def initialCountResponse = call read('classpath:helpers/favorites.feature')
        * def initialCount = initialCountResponse.count
        * print initialCount
        * if(initialCount == 1) karate.call('classpath:helpers/delete-favorite.feature')
        * def countResponse = call read('classpath:helpers/favorites.feature')
        ...
        * def finalCountResponse = call read('classpath:helpers/favorites.feature')
        * def finalCount = finalCountResponse.count
        And match finalCount == count + 1
        # Step 6: Get all favorite articles
        Given path 'articles'
        And params {favorited: johantest, limit: 10, offset: 0}
        And header Authorization = 'Token '+ token
        When method Get
        Then status 200
    ...
    Scenario: Comment articles
        * def commentGenerator = Java.type('helpers.DataGenerate')
